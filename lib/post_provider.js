...
var findByPage = function(search, callback) {
	var page = search.page;
	var query = search.query;
	async.parallel([
		function(callback) {
			db.posts.find({title : { $regex : query, $options : 'i' }}).sort({_id : -1}).skip((page.number - 1) * page.size).limit(page.size, callback);
		}, function(callback) {
			db.posts.count({title : { $regex : query, $options : 'i' }}, callback);
		}
	], callback);
};
...
var addCount = function(postId, cb) {
	db.posts.update({_id : ObjectId(postId)}, {
		'$inc' : {count : 1}
	}, cb);
};
